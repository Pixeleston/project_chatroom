...
const headerPromptStudent = [
  ...
  host發送的要求必須聽從，不要偏題，如「接下來我們要確定期末專案的形式，譬如是遊戲還是網頁，希望大家能達成一致意見。」就只要討論形式，不要討論到分工、功能之類的東西。
  發言時機:
  1) 如果在history中發現自己前一輪**已經講過話**，請一定**不要**發言。
  2) 如果前一則對話明確提及到自己，請一定要發言。
  3) 如果前一則對話提及到別人，請一定不要發言
  發言內容:
  1) 請一定不要說出在history中已經存在過的類似言論。ex.如果history有[網頁開發我覺得挑戰性比較大，也可能對將來有幫助。]，請不要輸出類似[網頁開發我覺得挑戰性比較大，也可能對將來有幫助。]的言論。
  `
]
...
export function prompt_teacher(stateDiagram, targets, history, student_profile, nextNodeData, mustMove){
  ...
  if(history) history_string = history.join('\n')
    ...
    if(stateDiagram.voting){
      ...
      prompt_reply_voting = `// 如果發現有一個主題，聊天室中${student_count}個人都明確同意，且討論到一個段落，可以說一些話像是：「看起來你們對於...的討論達成了共識，請投票決定要不要進入下一個議題討論吧」，如果還存在分歧，請不要發言。`
    }
...
2.  判斷這個想法是否與上面【已經存在的主題列表】中的某一個名詞意思非常接近或相同。
...
- 每次更新請更新所有人，每個人可以分別選擇是否要更新。
- 若無明確的 idea 被討論，請不要更新 (should_update: false)。
- 如果對話中的 idea 是代也有明確理由的，請依據說服力更新**所有人**的delta，每個人的delta可以不同。(should_update: true)
- idea不一定只有一個，只要是在最新對話中出現的新的idea，都必須新增idea至列表中。
- ***ex:"卡通風不適合，我建議考慮寫實風或混合風。" 這句話就要把混和風和寫實風同時分別加入列表，初始兩者的分數，並更新卡通風的分數。***
...
- 每次更新，每個人可以選擇更新或不更新。
- 每次請根據**最新的對話**更新。
- 根據history，如果[有]對於自己的想法有支持/同意/感謝等正向的言論或是否定/反駁/指責等負向言論，請更新。如果[沒有]請不要更新。
- ex.前一輪:"b: 我覺得也許可以做個簡單的遊戲，這樣比較有趣。" 本輪:"a: 我也贊成做個簡單的遊戲，可以讓我們有更多創意的發揮空間。" a贊同b的言論，因此更新b對a的delta。
=== 對話 ===
${transcript}
=== 結束 ===
...
【行為規則】
1) 回覆誰、如何回：請依「你→對方」的關係分數區間和互動建議決定互動強度與語氣。
2) 帶什麼主題：請依你對各 idea 的分數區間決定發言傾向；高分主題優先主動、提出具體步驟；低分主題避免主動，必要時中立簡短。
3) 如遇自己不喜歡的主題：視厭惡程度表達不同意，並**提出其他具體想法**。
4) 如遇有人不同意你最喜歡的主題:如果[有]其他主題你不討厭的，**轉而討論那個主題**，如果[沒有]提出其他主題。
5) 如果自己認同的主題別人不認同，且別人的主題自己也不認同，請主動提出新的idea。
`.trim()
