...
export function extractOutlinePrompt(outline){
  let prompt = `教師正在製作學生群組討論的大綱，裡面包含學生在討論時所需的各項流程以及詳細事項，以下為教師提供的大綱：
            ...
            請將具體的討論主題按照順序條列出來，如果大綱中有指定生成節點數請注意，主題請回傳重點，不要太長，請回傳一個陣列以<END>結尾
            範例：['確認主題', '細節討論', '總結報告']<END>
  `
  return prompt;
}
...
export function extractDetailPrompt(outline, nodeArray){
  ...
  let prompt = []
  let _prompt = `
    教師正在製作學生群組討論的大綱，裡面包含學生在討論時所需的各項流程以及詳細事項，以下為教師提供的大綱：
    ${outline}
